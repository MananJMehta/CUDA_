...
__constant__ float d_A[WIDTH*WIDTH];

__global__ void MatrixMul(float* B, float* C, unsigned long long* runtime)
{
...
for(int k = 0; k < WIDTH; k++)
    		{
        		C[row*WIDTH + column] += d_A[row*WIDTH+k] * B[k*WIDTH + column];
    		}
...
//    if (err != cudaSuccess)
 //   {
  ...
  //      exit(EXIT_FAILURE);
    ...
    err = cudaMemcpyToSymbol(d_A, h_A, WIDTH*WIDTH*sizeof(int));// FILL HERE
    if (err != cudaSuccess)
    {
    ...
    int threadsPerBlock = 32*32;// FILL HERE
	printf("CUDA kernel launch with %d blocks of %d threads\n", blocksPerGrid, threadsPerBlock);
	MatrixMul<<<blocksPerGrid,threadsPerBlock>>>(d_B, d_C, d_runtime);
    // <--                                                           
