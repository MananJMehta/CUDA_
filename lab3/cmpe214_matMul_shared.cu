...
__shared__ float sA[WIDTH][WIDTH]; 
__shared__ float sB[WIDTH][WIDTH];

sA[row][column] = A[row*WIDTH+column];
sB[row][column] = B[row*WIDTH+column];

__syncthreads();

for(int k = 0; k < WIDTH; k++)
    		{
        		lC += sA[row][k] * sB[k][column];
    		}

C[row*WIDTH+column] = lC;
