...
reduce0(int *g_idata, int *g_odata)
{
	...
	__shared__ int shared_tmp[1024];
	shared_tmp[tid] = g_idata[start];
	__syncthreads();
	...
	if (tid == 0)
		g_odata[blockIdx.x] = shared_tmp[0];
...
reduce2(int *g_idata, int *g_odata)
{
	...
	if (tid < 32)
		shared_tmp[tid] += shared_tmp[tid + 32];
	if (tid < 16)
		shared_tmp[tid] += shared_tmp[tid + 16];
	if (tid < 8)
		shared_tmp[tid] += shared_tmp[tid + 8];
	if (tid < 4)
		shared_tmp[tid] += shared_tmp[tid + 4];
	if (tid < 2)
		shared_tmp[tid] += shared_tmp[tid + 2];
	if (tid < 1)
		shared_tmp[tid] += shared_tmp[tid + 1];
...
reduce3(int *g_idata, int *g_odata)
{
	...
	if (blockSize >= 1024 && tid < 512)
		shared_tmp[tid] += shared_tmp[tid + 512];
	__syncthreads();

	if (blockSize >= 512 && tid < 256)
		shared_tmp[tid] += shared_tmp[tid + 256];
	__syncthreads();

	if (blockSize >= 256 && tid < 128)
		shared_tmp[tid] += shared_tmp[tid + 128];
	__syncthreads();

	if (blockSize >= 128 && tid < 64)
		shared_tmp[tid] += shared_tmp[tid + 64];
	__syncthreads();


	int mySum = shared_tmp[tid];

	if (tid < 32)
	{
		if (blockSize >= 64)
			mySum += shared_tmp[tid + 32];
